...
VkFFTConfiguration *make_config(const long *size, const size_t fftdim,
                                ...
                                const int useLUT, const int keepShaderCode, 
                                const size_t n_batch, const long *skip,
                                ...
                                const int singleKernelMultipleBatches)
{
    ...
    for (int i = 0; i < VKFFT_MAX_FFT_DIMENSIONS; i++)
        if (grouped_batch[i] > 0)
            config->groupedBatch[i] = grouped_batch[i];
    ...
    *pbuf = buffer;
    ...
    {
        if (dct || dst)
            *psize = (uint64_t)(s * precision);
        else
            *psize = (uint64_t)(s * precision * (size_t)2);
    }
    ...
    // cout << "performConvolution:" << config->performConvolution << endl;
    /*
                           ...
                           long *num_axis_upload)
{
    ...
    */
    ...
    for (int i = 0; i < VKFFT_MAX_FFT_DIMENSIONS; i++)
        use_bluestein_fft[i] = app->useBluesteinFFT[i];
    for (int i = 0; i < VKFFT_MAX_FFT_DIMENSIONS; i++)
...
void free_config(VkFFTConfiguration *config)
{
    ...
    if ((config->outputBuffer != NULL) && (config->buffer != config->outputBuffer))
        free((void *)config->outputBuffer);
    if ((config->inputBuffer != NULL) && (config->buffer != config->inputBuffer) && (config->outputBuffer != config->inputBuffer))
        free((void *)config->inputBuffer);

    if ((config->inputBufferSize != NULL) && (config->inputBufferSize != config->bufferSize))
        free((void *)config->inputBufferSize);
    if ((config->outputBufferSize != NULL) && (config->outputBufferSize != config->bufferSize) && (config->outputBufferSize != config->inputBufferSize))
        free((void *)config->outputBufferSize);

    if ((config->kernel != NULL) && config->performConvolution)
